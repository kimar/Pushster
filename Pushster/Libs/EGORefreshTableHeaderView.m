...
- (id)initWithFrame:(CGRect)frame {
    if (self == [super initWithFrame:frame]) {
		...
		[statusLabel release];
		UIImage *refreshArrow = [UIImage imageNamed:@"PullToRefresh.png"];
...
- (void)setLastUpdatedDate:(NSDate *)newDate {
	if (newDate) {
		...
		lastUpdatedLabel.text = [NSString stringWithFormat:@"Last update: %@", [lastUpdatedDate prettyDate]];
	} else {
		lastUpdatedDate = nil;
		lastUpdatedLabel.text = @"Last update: Never";
	}
}
...
- (void)setStatus:(int)status {
	switch (status) {
		case kReleaseToReloadStatus:
			statusLabel.text = @"Release for refresh...";
			break;
